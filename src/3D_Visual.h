...
public:
	...
	virtual void OnUpdate(Timestep ts) override
	{
		...
		for (const auto &polygon : m_Polygons)
		{
			...
			Frustum currentFrustum{m_Camera.GetViewProjectionMatrix()};
    ...
    virtual void OnImGuiRender() override
	{
		...
		ImGui::DragFloat("Camera Speed", &m_CameraMoveSpeed, 1.0f, 1.0f, 50.0f);
		...
		if (ImGui::DragFloat("Far", &far, far))
		{
			m_Camera.setFar(far);
		}
		ImGui::End();
...
private:
	...
	bool onMouseScrolled(const MouseScrolledEvent &e)
	{
		...
		if (m_CameraMoveSpeed <= 50.0f || m_CameraMoveSpeed >= 1.0f)
		{
			m_CameraMoveSpeed += yoffset;
		}
